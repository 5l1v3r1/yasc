...
std::string EnvironmentImpl::getImpl(const std::string& name)
{
	...
	std::string result;
	std::wstring wbuffer{ buffer.begin(), buffer.size() };
	result = wstr_to_utf8(wbuffer);
	return result;
}
...
std::string EnvironmentImpl::osVersionImpl()
{
	...
	std::string version;
	version = wstr_to_utf8(vi.szCSDVersion);
...
void EnvironmentImpl::nodeIdImpl(NodeId& id)
{
	...
	DWORD rc = pGetAdaptersInfo(pAdapterInfo, &len);
	...
	if (pGetAdaptersInfo(pAdapterInfo, &len) == NO_ERROR) 
	{
